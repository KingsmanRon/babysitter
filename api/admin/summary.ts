...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { data: products } = await db
      .from("products")
      .select("id, slug, name, stock_count, price_cents, currency, is_active, compare_at_price_cents, discount_percent_bps, sale_price_cents, sale_starts_at, sale_ends_at")
